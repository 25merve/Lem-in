...
func main() {
	...
	for {
		...
		// Her bir karınca için mevcut rotadaki bir sonraki konumu al
		for i := 0; i < antCount; i++ {
			// Karınca hedef konumuna ulaştıysa devam et

			// Karıncanın mevcut rotasındaki bir sonraki konumu al
			nextPositionIndex := -1
			...
			// Karıncanın rotası sona erdiyse devam et
			if nextPositionIndex >= len(antPaths[i]) {
				continue
			}

			// Karıncanın rotasındaki bir sonraki konumu al
