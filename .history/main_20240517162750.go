	...
	if len(paths) > 0 {
		...
		for {
			...
			for j := 0; j < antCount; j++ {
				if step < len(antPaths[j]) {
					currentPath := paths[j%len(paths)]
...
func main() {
	...
	if len(paths) > 0 {
			...
			for j := 0; j < antCount; j++ {
				if step < len(antPaths[j]) {
					...
					if step < len(currentPath) {
						nextNode := currentPath[step]
						if !occupied[nextNode] {
							antPaths[j] = append(antPaths[j], nextNode)
							occupied[nextNode] = true
						} else {
							alternativePathFound := false
							for _, path := range paths {
								if step < len(path) && !occupied[path[step]] {
									antPaths[j][step] = path[step]
									occupied[path[step]] = true
		...
		for step := 0; step < len(antPaths[0]); step++ {
			for j := 0; j < antCount; j++ {
				if step < len(antPaths[j]) && antPaths[j][step] != graph.EndNodeID {
					fmt.Printf("L%d-%d ", j+1, antPaths[j][step])
				}
			}
			fmt.Println()
		}
