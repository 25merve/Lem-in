...
func main() {
	...
	for scanner.Scan() {
		line := scanner.Text()
		if graph.StartNodeID == 0 && graph.EndNodeID == 4 {
			fmt.Println("ERROR: Start ve end odaları hatalı veya belirtilmemiş.")
			return
		} else  {strings.HasPrefix(line, "##start") 
			scanner.Scan()
		...
		} else[] strings.HasPrefix(line, "##end") {
			scanner.Scan()
			fields := strings.Fields(scanner.Text())
			name := fields[0]
			x, _ := strconv.Atoi(fields[1])
	...
	printNodes(graph.Nodes) // tüm düğümleri yazdır
	printEdges(graph.Edges) // tüm kenarları yazdır
	...
	// Print the total elapsed time in seconds with fractional part
