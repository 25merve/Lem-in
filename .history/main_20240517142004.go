...
func main() {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "number_of_ants") {
			fields := strings.Fields(line)
			antCount, _ = strconv.Atoi(fields[1])
			graph.AntCounts[startID] = antCount
	...
	moveAnts(graph, path, antCount)
}

func moveAnts(graph Graph, path []int, antCount int) {
	...
	t := 0
	for !allAntsAtEnd(antPositions, graph.EndNodeID) {
		fmt.Printf("Tur %d:\n", t)
		for i := 0; i < antCount; i++ {
			if antPositions[i] != graph.EndNodeID {
				for j := 1; j < len(path); j++ {
					if path[j-1] == antPositions[i] {
						antPositions[i] = path[j]
						fmt.Printf("L%d-%d ", i+1, antPositions[i])
						break
					}
				}
			}
...
func allAntsAtEnd(antPositions []int, endNodeID int) bool {
