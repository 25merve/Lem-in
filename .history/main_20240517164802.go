...
func main() {
	...
	if len(paths) > 0 {
		...
		// Her adımda, her bir karıncanın sadece bir odaya ilerlemesini sağlamak için
		for i := 1; i < len(paths[0]); i++ {
			for j := 0; j < antCount; j++ {
				if i < len(antPaths[j]) {
					currentPosition := antPaths[j][i]
					// Eğer bu odada başka bir karınca yoksa, ilerle
					if nextSteps[currentPosition] == 0 {
						fmt.Printf("L%d-%d ", j+1, currentPosition)
						nextSteps[currentPosition] = j + 1 // Karıncanın bu odada olduğunu belirt
					} else {
						// Bu odada başka bir karınca varsa, başka bir yola yönlendir
						nextRoom := paths[(j+1)%len(paths)][i]
						fmt.Printf("L%d-%d ", j+1, nextRoom)
						nextSteps[nextRoom] = j + 1 // Karıncanın bu odada olduğunu belirt
					}
				}
			}
			fmt.Println()
			// Bir sonraki adım için takip edilecek karınca pozisyonlarını sıfırla
			nextSteps = make(map[int]int)
		}
