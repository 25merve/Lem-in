...
adim :
for {
    // Hareketlerin hesaplanması ve yazdırılması
    fmt.Printf("Adım %d: %s\n", adim, strings.Join(hareketler, " "))
    
    // Adımı artır
    adim++

    // Hareketlerin hesaplanması ve yazdırılmasından sonra, her adımın işlemlerini belirten çıktıları yazdır
    for _, hareket := range hareketler {
        fmt.Printf("Adım %d: %s\n", adim, hareket)
    }

    // Döngü koşulları kontrol edilir ve gerekirse döngüden çıkılır
    if hepsiBitisteMi {
        break
    }
}
