...
// BFS kullanarak başlangıç ve bitiş düğümleri arasındaki tüm yolları bulur
func findAllPathsBFS(graph Graph, startID, endID int) [][]int {
	// Tüm yolları tutacak slice
	var allPaths [][]int

	// Ziyaret edilen düğümleri ve bu düğümlere giden yolları tutacak map
	...
	for len(queue) > 0 {
		...
		// Bitiş düğümünü bulduysak, bu yolu ekle ve devam et
		if node == endID {
			// Yolu oluştur
			path := []int{endID}
			for parent := paths[endID]; parent != startID; parent = paths[parent] {
		...
		for _, edge := range graph.Edges {
			if edge.Start == node && !visited[edge.End] {
				// Düğümü ziyaret edildi olarak işaretle
				visited[edge.End] = true
				// Düğümün ebeveynini tut
				paths[edge.End] = node
				// Kuyruğa ekle
...
// Tüm yolları bul
allPathsBFS := findAllPathsBFS(graph, startID, endID)

// Tüm yolları yazdır
fmt.Println("Tüm Yollar:")
for _, path := range allPathsBFS {
	fmt.Println(path)
}
}
