...
// Edmonds-Karp algoritması ile en kısa yolu bulur ve filtreleme işlemi yapar
func (g *Grafik) EdmondsKarp(baslangicDugumID int, bitisDugumID int, kapasiteler map[Kenar]int, filtreliDugumler map[int]bool) (maxAkis int, yol []int) {
	var enKisaYol []int
	maxAkis = 0

	// Ana döngü
	for {
		// BFS ile artan yolu bul
		enKisaYol = g.BFSFiltered(baslangicDugumID, bitisDugumID, kapasiteler, filtreliDugumler)
		if enKisaYol == nil {
			break // Artan yol yoksa sonlandır
		}

		// Artan yolu kullanarak akışı güncelle
		akis := g.artanYolAkisi(enKisaYol, kapasiteler)
		maxAkis += akis

		// Yeniden başlamak için artan yolu kullanarak kapasite tablosunu güncelle
		g.kapasiteTablosunuGuncelle(enKisaYol, akis, kapasiteler)
	}

	return maxAkis, enKisaYol
}

// Artan yolu kullanarak akışı hesaplar
func (g *Grafik) artanYolAkisi(yol []int, kapasiteler map[Kenar]int) int {
	akis := math.MaxInt64
	for i := 1; i < len(yol); i++ {
		kenar := Kenar{Baslangic: yol[i-1], Bitis: yol[i]}
		kapasite := kapasiteler[kenar]
		if kapasite < akis {
			akis = kapasite
		}
	}
...
func (g *Grafik) kapasiteTablosunuGuncelle(yol []int, akis int, kapasiteler map[Kenar]int) {
	for i := 1; i < len(yol); i++ {
		...
		tersKenar := Kenar{Baslangic: yol[i], Bitis: yol[i-1]}
		kapasiteler[tersKenar] += akis
	}
}

// BFSFiltered fonksiyonunu Edmonds-Karp algoritması için günceller
func (g *Grafik) BFSFiltered(baslangicDugumID int, bitisDugumID int, kapasiteler map[Kenar]int, filtreliDugumler map[int]bool) []int {
	...
	for len(kuyruk) > 0 {
		...
		for _, komsu := range g.KomusulukListesi[dugum] {
			...
			if !ziyaretEdildi[komsu] && !filtreliDugumler[komsu] && kapasiteler[kenar] > 0 {
