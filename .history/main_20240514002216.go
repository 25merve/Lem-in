...
// BFS kullanarak başlangıç düğümünden tüm diğer düğümlere olan tüm yolları bulur
func findAllPathsBFS(graph Graph, startID int) map[int][][]int {
	// Tüm yolları tutacak map
	...
	for len(queue) > 0 {
		...
		if currentNode != startID {
			allPaths[currentNode] = append(allPaths[currentNode], currentPath)
		}
...
// Tüm yolları bul
allPathsBFS := findAllPathsBFS(graph, startID)

// Tüm yolları yazdır
printAllPaths(allPathsBFS)
}
