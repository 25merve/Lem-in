...
func main() {
	...
	// Function to filter paths to avoid conflicts
...
func filterPaths(allPaths [][]int, antCount int) [][]int {
	filteredPaths := [][]int{}
	nodeOccupied := make(map[int]int) // node to step map

	for _, path := range allPaths {
		conflict := false
		for step, node := range path {
			if prevStep, ok := nodeOccupied[node]; ok && prevStep == step {
				conflict = true
				break
			}
		}
		if !conflict {
			...
			for step, node := range path {
				nodeOccupied[node] = step
			}
		}
	}

	if len(filteredPaths) < antCount {
		fmt.Println("Not enough unique paths found to avoid conflicts.")
		return [][]int{}
	}
