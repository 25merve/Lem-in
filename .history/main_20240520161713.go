...
func (g *Grafik) EdmondsKarp(baslangicDugumID, bitisDugumID int, kapasiteler map[Kenar]int, filtreliDugumler map[int]bool) (int, []int) {
	...
	for i := 0; i < len(yol)-1; i++ {
		...
		// Akış miktarı, minimum kapasiteye göre belirlenir
		akis := kapasiteler[kenar]
		if akis > 0 {
			maxAkis += akis
			// Kapasite tablosu güncellenir
			g.kapasiteTablosunuGuncelle(yol, akis, kapasiteler)
		}
	}
...
func main() {
	...
	// Edmonds-Karp algoritmasını kullanarak her karıncanın farklı bir yol izlemesini sağlar
	for i := 0; i < karincaSayisi; i++ {
		// En kısa yol ve maksimum akış miktarını al
		maxAkis, enKisaYol := grafik.EdmondsKarp(grafik.BaslangicDugumID, grafik.BitisDugumID, make(map[Kenar]int), make(map[int]bool))
		...
		// Karıncanın bu yolu izlemesi için kapasite tablosunu güncelle
		kapasiteler := make(map[Kenar]int)
		for _, kenar := range grafik.Kenarlari {
			kapasiteler[kenar] = 1 // Başlangıçta her kenarın kapasitesini 1 olarak ayarla
		}
		grafik.kapasiteTablosunuGuncelle(enKisaYol, 1, kapasiteler)
	}
}
