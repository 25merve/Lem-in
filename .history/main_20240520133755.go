...
// BFS algoritması ile en kısa yolu bulur ve filtreleme işlemi yapar
func (g *Grafik) BFSFiltered(baslangicDugumID int, bitisDugumID int, filtreliDugumler map[int]bool) []int {
	...
	for len(kuyruk) > 0 {
		...
		for _, komsu := range g.KomusulukListesi[dugum] {
			if !ziyaretEdildi[komsu] && !filtreliDugumler[komsu] {
...
func main() {
	...
	// Filtreleme için kullanılacak düğümleri belirleyin (örneğin, belirli düğümleri engellemek için)
	filtreliDugumler := make(map[int]bool)
	// Filtrelemek istediğiniz düğümleri belirleyin (filtreleme işlemi için koşullara göre)
	// Örneğin, burada düğüm 3'ü engelleyelim:
	filtreliDugumler[3] = true

	// BFSFiltered fonksiyonunu çağırarak filtrelenmiş en kısa yolu bulun
	enKisaYolFiltered := grafik.BFSFiltered(grafik.BaslangicDugumID, grafik.BitisDugumID, filtreliDugumler)
	if enKisaYolFiltered == nil {
		fmt.Println("Hata: Başlangıç ve bitiş arasında geçerli bir yol yok.")
		return
	}
	fmt.Println("Filtrelenmiş en kısa yol:", enKisaYolFiltered)
