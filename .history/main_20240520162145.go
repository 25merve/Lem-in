...
// Edmonds-Karp algoritması ile en kısa yolu bulur ve karıncaların bu yolu izlemesini sağlar
func (g *Grafik) EdmondsKarpForAnts(karincaSayisi int) {
	// Kapasiteleri belirlemek için bir harita oluştur
	kapasiteler := make(map[Kenar]int)
	for _, kenar := range g.Kenarlari {
		// Başlangıçta her kenarın kapasitesini 1 olarak ayarla
		kapasiteler[kenar] = 1
	}

	// Karıncaların her biri için Edmonds-Karp algoritmasını uygula
	for i := 0; i < karincaSayisi; i++ {
		// En kısa yol ve maksimum akış miktarını al
		maxAkis, enKisaYol := g.EdmondsKarp(g.BaslangicDugumID, g.BitisDugumID, kapasiteler, make(map[int]bool))

		// Eğer en kısa yol bulunamadıysa veya maksimum akış miktarı sıfırsa hata ver
		if enKisaYol == nil || maxAkis == 0 {
			fmt.Println("Hata: Başlangıç ve bitiş arasında geçerli bir yol yok.")
			return
		}

		// Karıncanın en kısa yolu izlemesini sağla
		fmt.Printf("Karıncanın %d. yolu: %v\n", i+1, enKisaYol)
	}
}
...
func (g *Grafik) EdmondsKarp(baslangicDugumID, bitisDugumID int, kapasiteler map[Kenar]int, filtreliDugumler map[int]bool) (int, []int) {
	// En kısa yolun bulunması için BFS kullanılır
	yol := g.BFS(baslangicDugumID, bitisDugumID)
	if yol == nil {
		return 0, nil
	}
	...
	// En kısa yol boyunca akış miktarı artırılır
	for {
		// BFS ile bulunan yolun üzerinde en küçük kapasiteyi bul
		minKapasite, bulundu := g.minKapasiteyiBul(yol, kapasiteler)
		if !bulundu {
			break
		}
		// Maksimum akışı güncelle
		maxAkis += minKapasite
		// Kapasite tablosunu güncelle
		g.kapasiteTablosunuGuncelle(yol, minKapasite, kapasiteler)
		// Yeni bir yol bul
		yol = g.BFS(baslangicDugumID, bitisDugumID)
		if yol == nil {
			break
