...
func main() {
	...
	if len(allPaths) == 0 {
		fmt.Println("ERROR: invalid data format") // Error: No valid path between start and end
		return
	}
	...
	for {
		...
		if step > 100 { // Adım sayısını kontrol altında tut
			break
		}
	}

	elapsed := time.Since(startTime) // Geçen süreyi hesapla
	if elapsed < 1.5*time.Minute {
		fmt.Println("Programın çalışması 1.5 dakikadan az sürdü.")
	} else {
		fmt.Println("Programın çalışması 1.5 dakikadan fazla sürdü.")
	}
}
