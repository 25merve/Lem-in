...
func main() {
	...
	endTime := time.Now()
	gecenSure := endTime.Sub(startTime)
...
func assignPathsToAnts(antCount int, filteredPaths [][]int) [][]int {
	...
	for i := 0; i < antCount; i++ {
		if i < len(remainingPaths) {
			antPaths[i] = remainingPaths[i]
		} else {
			antPaths[i] = remainingPaths[i%len(remainingPaths)]
		}
	}
...
// Function to filter paths to ensure no overlapping and optimal distribution
...
func overlap(path1, path2 []int) bool {
