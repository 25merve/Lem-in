...
func main() {
	...
	// Tüm yolları yazdır
	fmt.Println("1. odadan 0. odaya giden tüm yollar:")
	for _, path := range allPathsBFS {
		// Yolu tersine çevirerek başlangıçtan bitişe doğru yazdır
		reversedPath := reverseSlice(path)
		fmt.Println(reversedPath)
	}
}
// slice'ı tersine çevirir
