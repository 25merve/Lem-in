...
func alternatifYolOlustur(grafik *Grafik, karincaYollari [][]int, karincaKonumlari []int, doluOdalar []int) {
	// Dolu odalardan geçmeyen alternatif yolları bulmak için BFS kullanacağız
	for _, doluOda := range doluOdalar {
		// Dolu oda ile bitiş düğümü arasındaki alternatif yolu bulun
		alternatifYol := grafik.BFS(doluOda, grafik.BitisDugumID)
		if alternatifYol != nil {
			// Her bir karınca için alternatif yolu güncelle
			for i := 0; i < len(karincaYollari); i++ {
				// Karıncanın mevcut yolu dolu oda ile kesişiyorsa
				for j := 0; j < len(karincaYollari[i])-1; j++ {
					if karincaYollari[i][j] == doluOda {
						// Alternatif yolu mevcut yolun o noktasından başlayarak ekle
						karincaYollari[i] = append(karincaYollari[i][:j], append(alternatifYol, karincaYollari[i][j+1:]...)...)
						// Karıncanın yeni konumunu güncelle
						karincaKonumlari[i] = alternatifYol[0]
						break
					}
				}
			}
...
func main() {
	...
	// Karıncaların bulunduğu odaların sayısını belirler
	karincalarinBulunduguOdalar := make(map[int]bool)
	for _, konum := range karincaKonumlari {
		karincalarinBulunduguOdalar[konum] = true
	}

	turSayisi := 10 // Azaltılabilir tur sayısı
	adim := 1
	for tur := 0; tur < turSayisi; tur++ {
		hareketler := []string{}
		hepsiBitisteMi := true
		yerlesik := make(map[int]bool)

		// Dolu odaları belirle
		var doluOdalar []int
		for _, konum := range karincaKonumlari {
			doluOdalar = append(doluOdalar, konum)
		}
		// Alternatif yolları oluştur
		alternatifYolOlustur(&grafik, karincaYollari, karincaKonumlari, doluOdalar)
			...
			if karincaKonumlari[i] != grafik.BitisDugumID {
				...
				for j := 0; j < len(yol)-1; j++ {
					if yol[j] == karincaKonumlari[i] && !yerlesik[yol[j+1]] {
						...
						karincalarinBulunduguOdalar[karincaKonumlari[i]] = false // Eski konumundaki karıncayı kaldır
						karincaKonumlari[i] = yol[j+1]
						karincalarinBulunduguOdalar[karincaKonumlari[i]] = true // Yeni konumundaki karıncayı ekle
		...
		// Karıncaların bulunduğu odaları göster
		fmt.Printf("Adım %d:\n", adim)
		for konum, durum := range karincalarinBulunduguOdalar {
			if durum {
				fmt.Printf("Karıncaların konumu: %s\n", grafik.Dugumler[konum].Isim)
			}
		}
		fmt.Println("Hareketler:", strings.Join(hareketler, " "))
