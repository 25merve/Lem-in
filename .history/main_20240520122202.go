...
// BFS algoritması ile tüm yolları bulur
func (g *Grafik) BFSTumYollar(baslangicDugumID int, bitisDugumID int) [][]int {
	yollar := [][]int{}
...
// Bir dilimde öğe olup olmadığını kontrol eder
func iceriyorMu(dilim []int, oge int) bool {
	for _, v := range dilim {
		if v == oge {
			return true
		}
...
func main() {
	...
	tumYollar := grafik.BFSTumYollar(grafik.BaslangicDugumID, grafik.BitisDugumID)
	if len(tumYollar) == 0 {
		fmt.Println("Hata: Başlangıç ve bitiş arasında geçerli bir yol yok.")
		return
	}
	fmt.Println("BFS ile bulunan tüm yollar:", tumYollar)

	// Minimum sayıda yolu seçelim
	minYollar := tumYollar
	if len(tumYollar) > karincaSayisi {
		minYollar = tumYollar[:karincaSayisi]
	}

	karincaYollari := make([][]int, karincaSayisi)
	for i := 0; i < karincaSayisi; i++ {
		karincaYollari[i] = minYollar[i%len(minYollar)]
	}
