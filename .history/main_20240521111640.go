...
// Function to handle errors and exit the program with a message
...
func main() {
	...
	if len(os.Args) < 2 {
		exitWithError("Dosya adı belirtilmedi.")
	}

	file, err := os.Open(os.Args[1])
	if err != nil {
		exitWithError("Dosya açma hatası: " + err.Error())
	}
	defer file.Close()
	...
	if err != nil {
		exitWithError("Karınca sayısı okunamadı: " + err.Error())
	}
	if antCount <= 0 {
		exitWithError("ERROR: invalid data format:karınca sayısı 0 veya daha kücük olamaz.")
	} else {
	...
	if err := scanner.Err(); err != nil {
		exitWithError("Okuma hatası: " + err.Error()) // Reading error
	}

	if graph.StartNodeID == -1 || graph.EndNodeID == -1 {
		exitWithError("Hata: Başlangıç veya bitiş düğümü belirtilmemiş.") // Error: Start or end node not specified
	}
	...
	if len(filteredPaths) == 0 || len(filteredPaths[0]) == 0 {
		exitWithError("ERROR: invalid data format:Hata: Geçerli bir yol yok.") // Error: No valid path
	}
