...
func main() {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "##start") { //içinde start var mı diye kontrol ediyor
			...
			graph.Nodes = append(graph.Nodes, Node{ID: endID, X: endx, Y: endy}) // bitiş düğümünü düğümlere ekle			fmt.Println("Bitiş düğümü:", graph.EndNodeID)
...
// Dijkstra algoritması kullanarak en kısa üç yolu bulan fonksiyon
func findShortestPaths(graph Graph, startID, endID int, count int) [][]int {
	// Başlangıç düğümüne olan adım sayılarını saklayacak map
	steps := make(map[int]int)

	// En kısa yolu saklayacak slice
	var shortestPaths [][]int

	// Başlangıç düğümünün adım sayısını 0 olarak ayarla
	steps[startID] = 0

	// Öncelik kuyruğu (min-heap) oluştur
	priorityQueue := make(PriorityQueue, 0)
	heap.Init(&priorityQueue)
	heap.Push(&priorityQueue, &Item{value: startID, priority: 0})

	// Dijkstra algoritması
	for priorityQueue.Len() > 0 && len(shortestPaths) < count {
		// Kuyruğun en küçük öğesini al
		currentNode := heap.Pop(&priorityQueue).(*Item).value

		// Bitiş düğümüne ulaşıldıysa, en kısa yolu ekleyerek devam et
		if currentNode == endID {
			shortestPaths = append(shortestPaths, reconstructPath(steps, startID, endID))
			continue
		}
		...
		for _, edge := range graph.Edges {
			if edge.Start == currentNode {
				// Komşu düğümün yeni adım sayısı
				newSteps := steps[currentNode] + 1

				// Eğer daha önce görmemişssek veya daha kısa bir yol bulduysak
				if step, ok := steps[edge.End]; !ok || newSteps < step {
					steps[edge.End] = newSteps
					heap.Push(&priorityQueue, &Item{value: edge.End, priority: newSteps})
				}
			}
		}
	}
...
// Dijkstra algoritması sonrasında en kısa yolu yeniden oluşturmak için kullanılan yardımcı fonksiyon
func reconstructPath(steps map[int]int, startID, endID int) []int {
	path := []int{endID}
	currentNode := endID

	for currentNode != startID {
		for _, edge := range graph.Edges {
			if edge.End == currentNode && steps[edge.Start] == steps[currentNode]-1 {
				path = append([]int{edge.Start}, path...)
