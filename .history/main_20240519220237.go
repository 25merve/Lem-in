...
func main() {
	...
	// Function to check if an ant can move to the next node
	canMove := func(antPositions map[int]int, nextPos int, step int, pathLength int) bool {
	...
	for {
		...
		for i := 0; i < antCount; i++ {
			...
			currentPath := allPaths[i%len(allPaths)]
			currentPos := antPositions[i]

			nextPosIndex := -1
			for j, pos := range currentPath {
				if pos == currentPos {
					nextPosIndex = j + 1
					break
				}
			...
			if nextPosIndex >= len(currentPath) {
				continue
			}

			nextPos := currentPath[nextPosIndex]
			if canMove(antPositions, nextPos, step, len(currentPath)) {
				antPositions[i] = nextPos
				moves = append(moves, fmt.Sprintf("L%d-%d", i+1, nextPos))
				moved = true
			}
		}
