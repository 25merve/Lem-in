...
func main() {
	// Örnek verileri yükleme ve işleme
	ants, rooms, links, err := loadData()
	if err != nil {
		fmt.Println(err.Error()) // Sadece hata mesajını yazdır
		return
	}
...
func loadData() (int, []Room, []Link, error) {
	// Veri yükleme işlemleri
	// Hata durumunda, hata mesajı döndür
	return 0, nil, nil, fmt.Errorf("Veri yüklenirken hata")
}
