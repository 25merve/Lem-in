...
func (g *Graph) FindAllPaths(startNodeID int, endNodeID int) [][]int {
	var paths [][]int
	var dfs func(path []int, node int)
...
func main() {
	...
	paths := graph.FindAllPaths(graph.StartNodeID, graph.EndNodeID)
	fmt.Println("Paths found with DFS:", paths)
	...
	for {
		...
		for i := 0; i < antCount; i++ {
			...
			if !occupied[nextPosition] {
				...
				// Try to find an alternative path for the ant
				for _, path := range paths {
					for j, pos := range path {
						if pos == antPositions[i] {
							if j+1 < len(path) && !occupied[path[j+1]] {
								occupied[antPositions[i]] = false
								antPositions[i] = path[j+1]
								occupied[path[j+1]] = true
								moves = append(moves, fmt.Sprintf("L%d-%d", i+1, path[j+1]))
								moved = true
								break
							}
						}
