...
func main() {
	...
	if err := scanner.Err(); err != nil {
		fmt.Println("ERROR: invalid data format") // Reading error
		return
	}

	if graph.StartNodeID == -1 || graph.EndNodeID == -1 {
		fmt.Println("ERROR: invalid data format") // Error: Start or end node not specified
		return
	}
	...
	// Initialize the positions and paths for all ants
