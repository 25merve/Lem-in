...
func (g *Graph) printNodes() {
	fmt.Println("\n the_rooms:")
	for _, node := range g.Nodes {
		fmt.Printf("%d: (%d, %d) - Karınca Sayısı: %d\n", node.ID, node.X, node.Y, g.AntCounts[node.ID])
	}
}
...
func main() {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "##antcount") {
			scanner.Scan()
			antCount, _ = strconv.Atoi(scanner.Text())
			graph.AntCounts[startID] = antCount // Tüm karıncalar başlangıç odasında
