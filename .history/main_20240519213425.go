	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "##start") {
			...
			if len(fields) == 3 {
				...
				endName := edgeParts[1]
				startNodeID := findNodeIDByName(graph.Nodes, startName)
				endNodeID := findNodeIDByName(graph.Nodes, endName)
				graph.Edges = append(graph.Edges, Edge{Start: startNodeID, End: endNodeID})
				graph.AdjList[startNodeID] = append(graph.AdjList[startNodeID], endNodeID)
				graph.AdjList[endNodeID] = append(graph.AdjList[endNodeID], startNodeID)
			}
		}
	}
