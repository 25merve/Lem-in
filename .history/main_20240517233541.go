...
func main() {
	...
	for {
		moves := []string{} // Her adımda atılan adımların listesi
		moved := false      // En az bir karınca hareket etti mi?

		// Her bir karınca için en kısa yolu bul ve bir sonraki adımı al
		for i := 0; i < antCount; i++ {
			if antPositions[i] == graph.EndNodeID {
				continue
			}

			// Mevcut konumdan hedef konuma olan en kısa yolu bul
			shortestPath := graph.Dijkstra(antPositions[i])

			// Hedef konuma olan en kısa yolun bir sonraki adımını al
			nextNode := antPositions[i]       // Başlangıç düğümüyle başla
			minDistance := int(^uint(0) >> 1) // Max int
			for _, neighbor := range graph.AdjList[antPositions[i]] {
				if shortestPath[neighbor.End] < minDistance {
					nextNode = neighbor.End
					minDistance = shortestPath[neighbor.End]
				}
			}

			// Eğer bir sonraki düğüm boşsa, karıncayı oraya taşı
			if !occupied[nextNode] {
				// Eski konumu boşalt
				occupied[antPositions[i]] = false
				// Yeni konumu işgal et
				antPositions[i] = nextNode
				occupied[nextNode] = true
				// Hareketi kaydet
				moves = append(moves, fmt.Sprintf("L%d-%d", i+1, nextNode))
				moved = true
			}
		}

		// Eğer hiçbir karınca hareket etmediyse, döngüden çık
		if !moved {
			break
		}

		// Atılan adımları kaydet
