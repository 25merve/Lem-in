...
func main() {
	...
	for i := 0; i < 4; i++ {
		// Her turda karıncaların hareketlerini işle
		for scanner.Scan() {
			line := scanner.Text()
			if antCount == 0 { // Karınca sayısı daha önce belirlenmediyse
				...
				// Karıncaların hareketlerini oku ve işle
				...
				fmt.Printf("L%s-%d ", antName, antRoom)
			}
		}
		fmt.Println() // Her turun sonunda bir satır boşluk bırak
		// Her turda karıncaların konumlarını kontrol et
		for _, node := range graph.Nodes {
			if node.ID != graph.StartNodeID && node.ID != graph.EndNodeID {
				fmt.Printf("L%d-%d ", node.ID, graph.CheckAntCount(node.ID))
			}
		}
		fmt.Println() // Her turun sonunda bir satır boşluk bırak
		// Başlangıç ve bitiş düğümlerini kontrol et
		fmt.Printf("L%d-0 L%d-0\n", graph.StartNodeID, graph.EndNodeID)
	}
