...
func main() {
	...
	// Assign paths to ants dynamically based on current positions
	...
	for {
		...
		// Create a temporary list of occupied positions for the current step
		tempOccupied := make(map[int]bool)
		for pos := range occupied {
			tempOccupied[pos] = true
		}
		...
		for i := 0; i < antCount; i++ {
			...
			if !tempOccupied[nextPosition] {
				tempOccupied[antPositions[i]] = false
				antPositions[i] = nextPosition
				tempOccupied[nextPosition] = true
