...
func main() {
	...
	if antCount <= 0 {
		...
		// Print the number of ants to buffer
		outputBuffer.WriteString(fmt.Sprintf("Number of ants: %d\n", antCount))
	}
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "##start") {
			...
			outputBuffer.WriteString(fmt.Sprintf("start_room: %s\n", name))
			...
			outputBuffer.WriteString(fmt.Sprintf("end_room: %s\n", name))
...
// Function to filter paths ensuring no overlap among the paths
...
// Function to check if two paths overlap
func pathsOverlap(path1, path2 []int) bool {
