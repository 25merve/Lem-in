...
// BFSAllPaths finds all paths from startNodeID to endNodeID using BFS
...
// findNodeIDByName finds the node ID by its name
...
func main() {
	...
	if antCount <= 0 {
		fmt.Println("ERROR: invalid data format: karınca sayısı 0 veya daha küçük olamaz.")
		return
	} else {
	...
	gecenSure := time.Since(startTime)

	// Convert to minutes
	gecenSureDakika := gecenSure.Minutes()

	fmt.Printf("Toplam süre: %.1f dakika\n", gecenSureDakika)
}

// assignPathsToAnts assigns paths to ants ensuring no overlap and choosing the shortest path for the first ant, next shortest for the second, and so on.
func assignPathsToAnts(antCount int, paths [][]int) [][]int {
...
// filterPaths filters out overlapping paths
func filterPaths(paths [][]int, antCount int) [][]int {
	filteredPaths := [][]int{}
	nodeUsage := make(map[int]bool)
