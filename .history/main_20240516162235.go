...
for _, edge := range graph.Edges {
	...
	if !isBiDirectional(graph, edge.Start, edge.End) {
		dist[edge.End][edge.Start] = edge.Weight
	}
}

// Belirli iki düğüm arasında iki yönlü bir kenar olup olmadığını kontrol eder
func isBiDirectional(graph Graph, start, end int) bool {
	for _, edge := range graph.Edges {
