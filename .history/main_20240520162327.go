...
func (g *Grafik) EdmondsKarpForAnts(karincaSayisi int) {
	...
	for i := 0; i < karincaSayisi; i++ {
		...
		maxAkis, enKisaYol := g.EdmondsKarpForAnts(g.BaslangicDugumID, g.BitisDugumID, kapasiteler, make(map[int]bool))
...
// BFS ile bulunan yolun üzerinde en küçük kapasiteyi bulur
func (g *Grafik) minKapasiteyiBul(yol []int, kapasiteler map[Kenar]int) (int, bool) {
	minKapasite := math.MaxInt64
	bulundu := false

	for i := 0; i < len(yol)-1; i++ {
		kenar := Kenar{Baslangic: yol[i], Bitis: yol[i+1]}
		if kapasiteler[kenar] < minKapasite {
			minKapasite = kapasiteler[kenar]
			bulundu = true
		}
	}

	return minKapasite, bulundu
}
