...
func main() {
	...
	// BFS kullanarak başlangıç ve bitiş düğümleri arasındaki yolu bul
	path := findPathBFS(graph, startID, endID)
	if len(path) > 0 {
		fmt.Println("\nBFS Yolu:")
		...
		fmt.Println("\nBaşlangıç ve bitiş düğümleri arasında yol bulunamadı.")
	}
