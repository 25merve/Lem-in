...
func main() {
	graph := Graph{}
	graph.Nodes = append(graph.Nodes, Node{ID: 1, X: 0, Y: 0})
	graph.Nodes = append(graph.Nodes, Node{ID: 2, X: 1, Y: 1})
	graph.Nodes = append(graph.Nodes, Node{ID: 3, X: 2, Y: 2})
	graph.Nodes = append(graph.Nodes, Node{ID: 4, X: 3, Y: 3})
	graph.Nodes = append(graph.Nodes, Node{ID: 5, X: 4, Y: 4})

	graph.Edges = append(graph.Edges, Edge{Start: 1, End: 2})
	graph.Edges = append(graph.Edges, Edge{Start: 2, End: 3})
	graph.Edges = append(graph.Edges, Edge{Start: 3, End: 4})
	graph.Edges = append(graph.Edges, Edge{Start: 4, End: 5})

	startID := 1
	endID := 5

	shortestPath := shortestPathBFS(graph, startID, endID)
	fmt.Println("Başlangıçtan hedefe olan en kısa yol:", shortestPath)
}
