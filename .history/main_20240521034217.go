...
type Graph struct {
	Nodes       []Node        // Grafiktaki düğümlerin listesi
	Edges       []Edge        // Grafiktaki kenarların listesi
	StartNodeID int           // Başlangıç düğümünün ID'si
	EndNodeID   int           // Bitiş düğümünün ID'si
	AdjList     map[int][]int // Grafiği temsil eden komşuluk listesi
}
...
// startNodeID'den endNodeID'ye kadar tüm yolları BFS kullanarak bulan fonksiyon
...
// Belirli bir öğeyi bir dilimde içerip içermediğini kontrol eden yardımcı fonksiyon
...
// Adıyla düğümün ID'sini bulan fonksiyon
...
func main() {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "##start") {
			...
			fmt.Println("Başlangıç odası:", name) // Başlangıç odasını yazdır
			...
			fmt.Println("Bitiş odası:", name) // Bitiş odasını yazdır
	...
	if err := scanner.Err(); err != nil {
		fmt.Println("Okuma hatası:", err) // Okuma hatası
		return
	}

	if graph.StartNodeID == -1 || graph.EndNodeID == -1 {
		fmt.Println("Hata: Başlangıç veya bitiş düğümü belirtilmemiş.") // Hata: Başlangıç veya bitiş düğümü belirtilmemiş
		return
	}
