...
func main() {
	...
	for {
		...
		for i := 0; i < antCount; i++ {
			if antPositions[i] == graph.EndNodeID { // Karınca hedef konumuna ulaştıysa devam et

				continue
			}
			nextPositionIndex := -1 // Karıncanın mevcut rotasındaki bir sonraki konumu al
			...
			if nextPositionIndex >= len(antPaths[i]) { // Karıncanın rotası sona erdiyse devam et
