...
// Function to print all nodes (commented out for error-only output)
...
// Function to print all edges (commented out for error-only output)
...
func main() {
	...
	if antCount <= 0 {
		...
		// Print the number of ants (commented out for error-only output)
	...
	for {
		...
		// Print the moves for this step (commented out for error-only output)
		// if len(moves) > 0 {
	...
	geçensureDakika := float64(geçensure.Minutes())
