...
func main() {
	...
	for {
		...
		for i := 0; i < antCount; i++ {
			...
			// Mevcut konumdan hedef konuma olan en kısa yolu hesapla
			shortestPath := graph.Dijkstra(antPositions[i])

			// Mevcut konumdan hedef konuma olan en kısa yolun bir sonraki adımını al
			nextPosition := shortestPath[antPaths[i][0]]

			// Eğer bir sonraki konum boşsa, karıncayı oraya taşı
